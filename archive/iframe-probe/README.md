# mcp-apps-iframe-probe（アーカイブ）

> このディレクトリは本リポジトリの前身「MCP Apps iframe 対応状況の調査サーバー」のアーカイブ。
> ソースはビルド対象外で、本文中の相対パス・コマンドは当時のリポジトリ構成を前提としている。
> 動かす場合は `src/` 相当（index.ts, server.ts, probes.ts, view/）を tsconfig の include に戻すこと。
